# 👋 Hi, I'm Abdelrahman Elgebaly  

💻 **Frontend Developer (React)**  
Passionate about building modern, responsive, and user-friendly web applications. Always eager to learn new technologies and improve my skills.  

---

## 🚀 About Me
- 🌱 Currently focusing on **React, UI/UX, and E-Commerce projects**  
- 🔭 Building projects to enhance my portfolio and gain real-world experience  
- 📍 Based in Egypt, open to global opportunities  

---

## 🛠️ Tech Stack  

<p align="center">
  <!-- Languages -->
  <img src="https://skillicons.dev/icons?i=html,css,js" />
</p>

<p align="center">
  <!-- Frameworks & Libraries -->
  <img src="https://skillicons.dev/icons?i=react,bootstrap" />
</p>

<p align="center">
  <!-- Tools -->
  <img src="https://skillicons.dev/icons?i=git,github,vscode" />
</p>

---

## 📂 Featured Projects  

### 🛒 [Chefaa E-Commerce](https://github.com/Gebaaly/chefaa)  
A full-featured **online pharmacy web app** built with **React (frontend)** and **Laravel (backend)**.  
- Product browsing, categories, and cart functionality  
- Authentication and order confirmation  
- Responsive and user-friendly UI  

---


### 🌐 [Portfolio Website](https://github.com/Gebaaly/portfolio)  
A personal portfolio website to showcase my **skills, projects, and experience**.  
- Built with **React + Bootstrap + Sass**  
- Modern, responsive design  
- Includes animations and smooth UI  

---

### 📊 [Salary Survey](https://github.com/Gebaaly/Salary-Survey)  
A web app to **collect and visualize salary survey data**.  
- Built with **JavaScript**  
- Interactive survey form  
- Data visualization with charts  

---

### 💡 [Gebaaly](https://github.com/Gebaaly/gebaaly)  
A practice repository for **testing, learning, and experimenting** with new ideas.  
- Small code snippets and experiments  
- Focus on **JavaScript & React**  

---

## 📫 Connect with Me  
[![Email](https://img.shields.io/badge/Email-abdoalgebaly2003@gmail.com-red?style=for-the-badge&logo=gmail&logoColor=white)](mailto:abdoalgebaly2003@gmail.com)  
[![GitHub](https://img.shields.io/badge/GitHub-Gebaaly-181717?style=for-the-badge&logo=github)](https://github.com/Gebaaly)  
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Profile-blue?style=for-the-badge&logo=linkedin)](#)  
